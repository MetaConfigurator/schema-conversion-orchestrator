...
from schema_conversion_orchestrator.converters.external_process import ConverterExternalGeneric
from schema_conversion_orchestrator.domain.schema_types import SchemaLanguage

REPO_ROOT = Path(__file__).resolve().parents[3]
EXTERNAL_CONVERTERS_DIR = REPO_ROOT / "external_converters"
...
def register_external_converters() -> List[Converter]:
    ...
    # Node.js service
    node_executable = EXTERNAL_CONVERTERS_DIR / "node" / "dist" / "index.js"
    if node_executable.exists():
        print(f"Found Node.js converter at: {node_executable}")
        converter_infos = get_external_converter_info(str(node_executable), "node")
    ...
    # Java service
    java_jar = EXTERNAL_CONVERTERS_DIR / "java" / "converter.jar"
    if java_jar.exists():
        print(f"Found Java converter at: {java_jar}")
        converter_infos = get_external_converter_info(str(java_jar), "java")
